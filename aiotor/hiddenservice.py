...
from aiotor.utils import AuthType
from aiotor.parsers import IntroPointParser, HSDescriptorParser
from aiotor.crypto_common import sha1, aes_update, aes_ctr_decryptor, b64decode, curve25519_public_from_bytes
...
class HiddenService:
    ...
    def _parse_onion(self, onion_address):
        ...
        if onion_address.endswith('.onion'):
            onion_address = onion_address[:-6][-16:]
        ...
        if len(onion_address) == self.REND_SERVICE_ID_LEN_BASE32:
            ...
            # checksum =
            #   decoded[self.ED25519_PUBKEY_LEN:self.ED25519_PUBKEY_LEN + self.HS_SERVICE_ADDR_CHECKSUM_LEN_USED]
            ...
            raise Exception(f'Unknown onion address format:  {onion_address}')
...
class HiddenServiceConnector:
    ...
    async def get_responsibles_dir(self, hidden_service):
        i = 0
        async for responsible_router in self._consensus.get_responsibles(hidden_service):
...
class ResponsibleDir:
    ...
    async def get_introductions(self, hidden_service):
        descriptor_id = hidden_service.get_descriptor_id(self.replica)
        response = await self._fetch_descriptor(descriptor_id)
        async for intro_point in self._get_intro_points(response, hidden_service.descriptor_cookie):
            yield intro_point

    async def _fetch_descriptor(self, descriptor_id):
        # tor ref: rend_client_fetch_v2_desc
        # tor ref: fetch_v3_desc

        logger.info('Create circuit for hsdir')
        async with await self._circuit.create_new_circuit(extend_routers=[self._router]) as directory_circuit:
            assert directory_circuit.nodes_count == 2

            async with await directory_circuit.create_dir_client() as dir_client:
                ...
                status, response = await dir_client.get(descriptor_path)
    ...
    async def _info_to_router(self, intro_point_info):
        onion_router = await self._consensus.get_router(intro_point_info['introduction_point'])
        onion_router.service_key = intro_point_info['service_key']
        onion_router.onion_key = intro_point_info['onion_key']
        return onion_router

    async def _get_intro_points(self, response, descriptor_cookie):
        ...
        for intro_point_info in intro_points_info_list:
            router = await self._info_to_router(intro_point_info)
...
class IntroductionPoint:
    ...
    async def connect(self, hidden_service, rendezvous_cookie):
        # Create introduction point circuit
        async with await self._circuit.create_new_circuit(extend_routers=[self._introduction_router]) as intro_circuit:
            assert intro_circuit.nodes_count == 2

            # TODO: tor ref: v3 hs_client send_introduce1
            # Send Introduce1
            extend_node = await intro_circuit.rendezvous_introduce(
                self._circuit,
                rendezvous_cookie,
            ...
            rendezvous2_cell = await self._circuit.wait_rendezvous2()
