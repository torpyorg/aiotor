...
from aiotor.guard import guard_connect
from aiotor.property import async_cached_property
from aiotor.documents import TorDocumentsFactory
from aiotor.cache_storage import TorCacheDirStorage
from aiotor.crypto_common import rsa_verify, rsa_load_der
from aiotor.documents.network_status import RouterFlags, NetworkStatusDocument, FetchDescriptorError, Router
from aiotor.documents.dir_key_certificate import DirKeyCertificateList
from aiotor.documents.network_status_diff import NetworkStatusDiffDocument
from aiotor.parsers import RouterDescriptorParser
...
class TorConsensus:
    ...
    async def renew(self, force=False):
        async with self._lock:
            ...
            raw_string = await self.download_consensus(prev_hash)
            ...
            verified, signing_idents = await self.verify(new_doc)
            if not verified:
                await self.renew_certs(signing_idents)

                # Try verify again
                verified, _ = await self.verify(new_doc)
                if not verified:
    ...
    @retry(3, BaseException,
           log_func=functools.partial(log_retry, msg='Retry with another authority...'))
    async def renew_certs(self, signing_idents):
        key_certificates_raw = await self.download_public_keys(signing_idents)
        certs = DirKeyCertificateList(key_certificates_raw)
        self._certs = certs
        self._cache_storage.save_document(certs)

    async def get_router(self, fingerprint) -> Router:
        # TODO: make mapping with fingerprint as key?
        fingerprint_b = b32decode(fingerprint.upper())
        doc = await self.document
        return next(onion_router for onion_router in doc.routers if onion_router.fingerprint == fingerprint_b)

    async def get_routers(self, flags=None, has_dir_port=True):
        """
        ...
        doc = await self.document
        for onion_router in doc.routers:
    ...
    async def get_random_router(self, flags=None, has_dir_port=None):
        """
        ...
        :return: router
        """
        routers = await self.get_routers(flags, has_dir_port)
        return random.choice(routers)

    async def get_random_guard_node(self, different_flags=None):
        flags = different_flags or [RouterFlags.Guard]
        return await self.get_random_router(flags)

    async def get_random_exit_node(self):
        flags = [RouterFlags.Fast, RouterFlags.Running, RouterFlags.Valid, RouterFlags.Exit]
        return await self.get_random_router(flags)

    async def get_random_middle_node(self):
        flags = [RouterFlags.Fast, RouterFlags.Running, RouterFlags.Valid]
        return await self.get_random_router(flags)

    async def get_hsdirs(self):
        flags = [RouterFlags.HSDir]
        return await self.get_routers(flags, has_dir_port=True)

    async def _create_dir_circuit(self, authority=True, purpose=None):
        if authority:
            router = self._authorities.get_random()
        else:
            router = await self.get_random_router(has_dir_port=True)

        # tor ref: directory_get_from_dirserver DIR_PURPOSE_FETCH_CONSENSUS
        # tor ref: directory_send_command
        guard = await guard_connect(router, purpose=purpose)
        return guard, await guard.create_circuit(0)

    @async_cached_property
    async def _auth_dir_circuit(self):
        self._auth_guard, circuit = await self._create_dir_circuit(authority=True,
                                                                   purpose='Consensus/PublicKeys downloader')
        return circuit

    async def _get_auth_dir_client(self):
        auth_dir_circuit = await self._auth_dir_circuit
        return await auth_dir_circuit.create_dir_client()

    @async_cached_property
    async def _dir_circuit(self):
        self._guard, circuit = await self._create_dir_circuit(authority=False, purpose='Router descriptor downloader')
        return circuit

    async def _get_dir_client(self):
        dir_circuit = await self._dir_circuit
        return await dir_circuit.create_dir_client()
    ...
    async def download_consensus(self, prev_hash=None):
        logger.info('Downloading new consensus...')
        headers = {'X-Or-Diff-From-Consensus': prev_hash} if prev_hash else None
        async with await self._get_auth_dir_client() as dir_client:
            _, body = await dir_client.get(self.consensus_url, headers=headers)
            return body.decode()
    ...
    async def download_public_keys(self, signing_idents):
        ...
        async with await self._get_auth_dir_client() as dir_client:
            _, body = await dir_client.get(url)
            return body.decode()
    ...
    async def get_descriptor(self, fingerprint):
        """
        ...
        try:
            async with await self._get_dir_client() as dir_client:
                status, response = await dir_client.get(url)
            if status != 200:
    ...
    async def get_responsibles(self, hidden_service):
        """
        ...
        :return:
        """
        hsdir_router_list = await self.get_hsdirs()
